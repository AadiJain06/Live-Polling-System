...
io.on('connection', (socket) => {
  ...
  socket.on('remove-student', (userId) => {
    console.log(`Teacher ${socket.userName} attempting to remove student with ID: ${userId}`);
    ...
    if (studentToRemove && studentSocketId) {
      ...
      console.log(`Student ${studentToRemove.name} (${userId}) has been kicked out`);
    } else {
...
function endPoll() {
  if (currentPoll) {
    ...
    console.log(`Poll "${currentPoll.question}" added to history`);
